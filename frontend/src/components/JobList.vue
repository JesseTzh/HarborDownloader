...
<template>
  <section class="card">
    <header class="card-head summary-head">
      <div>
        <h2>下载任务</h2>
        <p class="hint summary-meta">一个任务可包含多个镜像，触发后按顺序一次性下载并导出 TAR。下载位置使用设置中的默认根目录。</p>
      </div>
      <button class="btn primary" :disabled="disabled" type="button" @click="emit('create')">新建任务</button>
    </header>
    ...
    <ul v-else class="job-list">
      <li v-for="job in jobs" :key="job.id" class="job-item">
        <div class="job-item-main">
          ...
          <p v-if="downloadRoot" class="hint mono wrap">{{ downloadRoot }}/{{ job.name }}</p>
          <ul class="job-images">
        ...
        <div class="job-item-actions">
          <button class="btn" :disabled="disabled || !(job.images || []).length" type="button" @click="emit('start', job)">
            下载
          </button>
          <button class="btn ghost" :disabled="disabled" type="button" @click="emit('edit', job)">编辑</button>
          <button class="btn ghost danger-text" :disabled="disabled" type="button" @click="emit('remove', job)">删除</button>
        </div>
      </li>
    </ul>
  </section>
